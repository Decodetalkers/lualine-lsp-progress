...
LspProgress.init = function(self, options)
    ...
    self.options.spinner_symbols = vim.tbl_extend('force', LspProgress.default.spinner_symbols,
    ...
    if self.options.colors.use then
        self.highlights.title = highlight.create_component_highlight_group({ fg = self.options.colors.title },
            'lspprogress_title', self.options)
        self.highlights.percentage = highlight.create_component_highlight_group({ fg = self.options.colors.percentage },
            'lspprogress_percentage', self.options)
        self.highlights.message = highlight.create_component_highlight_group({ fg = self.options.colors.message },
            'lspprogress_message', self.options)
        self.highlights.spinner = highlight.create_component_highlight_group({ fg = self.options.colors.spinner },
            'lspprogress_spinner', self.options)
        self.highlights.lsp_client_name = highlight.create_component_highlight_group(
            { fg = self.options.colors.lsp_client_name }, 'lspprogress_lsp_client_name', self.options)
    end
                ...
                if progress[i] and progress[i] ~= '' then
                    if options.colors.use then
                        table.insert(d,
                            ...
                            options.separators[i].pre .. progress[i] .. options.separators[i].post)
                    else
